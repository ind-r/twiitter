...
export const getSharesOfTweet = async (
  ...
  try {
    ...
    const tweetsCount: number | null = await Share.countDocuments({ tweetId });
...
export const getLikesOfTweet = async (
  ...
  try {
    ...
    const tweetsCount: number | null = await Like.countDocuments({ tweetId });
    if (tweetsCount) {
      return tweetsCount;
    }
    // console.log("tweet not found");
...
export const like = async (
  ...
  try {
    ...
    if (!connect) {
      console.log("Db not connected");
      return;
      // return { status: 500, message: "Db not
    }
    ...
    if (like) {
      ...
      // LIKE
      if (tweet && user) {
        const newLike: ILike = await new Like({
          userId,
          tweetId,
        });
        await newLike.save();
...
export const share = async (
  ...
  try {
    ...
    if (shareExists) {
      ...
      // SHARE
      const newShare: IShare = new Share({ userId, tweetId });
      await newShare.save();
